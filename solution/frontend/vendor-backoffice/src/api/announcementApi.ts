...
export async function uploadImage(file: File): Promise<ImageUploadResult> {
  ...
  const res = await authedFetch('/api/vendor/announcements/images', { method: 'POST', body: form })
...
export async function listAnnouncements(): Promise<Announcement[]> {
  const res = await authedFetch('/api/vendor/announcements')
...
export async function updateAnnouncement(id: string, payload: AnnouncementPayload): Promise<Announcement> {
  const res = await authedFetch(`/api/vendor/announcements/${id}`, {
    method: 'PUT',
    headers: JSON_HEADERS,
    body: JSON.stringify(payload),
  })
...
export async function deleteAnnouncement(id: string): Promise<void> {
  const res = await authedFetch(`/api/vendor/announcements/${id}`, { method: 'DELETE' })
...
export async function moveAnnouncementUp(id: string): Promise<Announcement> {
  const res = await authedFetch(`/api/vendor/announcements/${id}/move-up`, { method: 'PATCH' })
...
export async function moveAnnouncementDown(id: string): Promise<Announcement> {
  const res = await authedFetch(`/api/vendor/announcements/${id}/move-down`, { method: 'PATCH' })
  if (!res.ok) throw new Error('move_failed')
  return res.json()
}
