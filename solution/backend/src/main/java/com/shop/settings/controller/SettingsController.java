...
public interface SettingsController {
    ...
    /**
     * Returns the plain-text content of the requested legal page (public endpoint).
     *
     * @param key legal page identifier (e.g. {@code legal_cgv})
     * @return 200 with content, or 404 if the key is unknown
     */
    @Operation(summary = "Get legal page content (public)")
    @ApiResponse(responseCode = "200", description = "Content returned")
    @ApiResponse(responseCode = "404", description = "Unknown legal page key")
    @GetMapping("/api/public/legal/{key}")
    ResponseEntity<LegalPageResponse> getLegalPage(@PathVariable String key);
}
